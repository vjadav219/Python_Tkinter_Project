...
def date():         
    ...
    def date():
        ...
        t=de.entry.get()
    b1=ttk.Button(root,text="choose date",command=lambda:date())
    b1.pack(padx=2)
    ...
    def check():
        ...
        for i,j,k in zip(new,ab,number):
            if(i.get()==1):
                connection=mysql.connector.connect(host="localhost",user="root",password="",database="python_final_project")
                mycursor=connection.cursor()
                mycursor.execute("INSERT INTO class_a VALUES (%s, %s,%s,%s)", (k, j,"present",t))
                connection.commit()
            else:
                connection=mysql.connector.connect(host="localhost",user="root",password="",database="python_final_project")
                mycursor=connection.cursor()  
                mycursor.execute("INSERT INTO class_a VALUES (%s, %s,%s,%s)", (k, j,"absent",t))          
                connection.commit()
...
def attandance_B():
    ...
    def check():
        ...
        for i,j,k in zip(new,ab,number):
            if(i.get()==1):
                connection=mysql.connector.connect(host="localhost",user="root",password="",database="python_final_project")
                mycursor=connection.cursor()
                mycursor.execute("INSERT INTO class_b VALUES (%s, %s,%s,%s)", (k, j,"present",t))
                connection.commit()
            else:
                connection=mysql.connector.connect(host="localhost",user="root",password="",database="python_final_project")
                mycursor=connection.cursor()  
                mycursor.execute("INSERT INTO class_b VALUES (%s, %s,%s,%s)", (k, j,"absent",t))          
                connection.commit()
        tmsg.showinfo("attandance","Saved Successfully")
    ...
    ab=["AGHARA PRINCE VASUDEV","BAMBHANIYA RUTVI VITHTHALBHAI","AMRUTIYA NIRBHAY RAJESHBHAI","ASODARIYA BHAKTI PARESHBHAI","JAMBUKIYA TIRTH DILIPBHAI","BARAIYA ZENIT VINESHBHAI","KACHA DARSHAN JIVANBHAI","BHADRASIYA ALVAZ SALIMBHAI","BHALALA PRITESH NARENDRABHAI","KHOYANI RUSHANGI ARVINDBHAI","BHANDERI SUJAL AMITBHAI","KUSHVAHA SUMIT SHIVPRASAD","BHATT VATSAL ABHAYKUMAR","MESANIYA TAZEEM RIZVAN","CHAUDHARI DENISHA AMARSHIBHAI","SHERASHIYA MOHMMAD DANISH ISMAIL","THUMMAR ARPIT PARESHBHAI","UPARA DHRUV RAVIBHAI","DAMASIYA SOHIL BHARATBHAI","DESAI SHRUTI MANSUKHBHAI","DHAKHADA SHITAL ASHOKBHAI","VADAVIYA SAFIR IDRISBHAI","DUDHAT JENIL ASHOKBHAI","VADODARIYA JAIVIN MAHENDRABHAI","VADODARIYA VIVEK PRAVINBHAI","VAGHASIYA DHRUVI SHAILESHBHAI","VASOYA PARAS VIPULBHAI","GALANI KISHAN DINESHBHAI","VEKARIYA VENISH BHAVESHBHAI","VORA AYUSH SANJAYBHAI","GOLANI YASH MUKESHBHAI","VAISHNAV KRUPALI VISHALBHAI","KUMBHANI AKSHA MAHESHBHAI","LUNAGARIYA JEVIN VIPULBHAI","MADALIYA ARPITA PRASHANTBHAI","TIMBADIYA PRIYANSHI DHARMESHBHAI","MEGHPARA SUJAL DINESHBHAI","SIPAI SAMIRAHMAD MAHEBUBBHAI","MORAD NENCY ASHOKBHAI","MULIYA MAHEK RAJESHBHAI"]    
    k=5
    for i,j in zip(ab,new):
        i=ttk.Checkbutton(second_frame,cursor="plus", text =i, variable=j,onvalue = 1,bootstyle=SUCCESS)
        i.grid(row=k,column=10,sticky=W,pady=2)
    ...
    def check():
        ...
        for i,j,k in zip(new,ab,number):
            if(i.get()==1):
                connection=mysql.connector.connect(host="localhost",user="root",password="",database="python_final_project")
                mycursor=connection.cursor()
                mycursor.execute("INSERT INTO class_c VALUES (%s, %s,%s,%s)", (k, j,"present",t))
                connection.commit()
            else:
                connection=mysql.connector.connect(host="localhost",user="root",password="",database="python_final_project")
                mycursor=connection.cursor()  
                mycursor.execute("INSERT INTO class_c VALUES (%s, %s,%s,%s)", (k, j,"absent",t))          
                connection.commit()
        tmsg.showinfo("attandance","Saved Successfully")
    ab=Label(second_frame,text="Unclick To Absent Student",justify=CENTER,padx=14,font="verdana  15 bold")
    ab.grid(row=1,column=10,pady=10)
    ab=["MUNGAPARA MEET SANJAYBHAI","NALIYADHARA JENISH BHARATBHAI","SAVALIYA NIRMESH BHARATBHAI","NONGHANAVADARA DENISHA NILESHBHAI","PADHIYAR PRADHYUMANSINH VISHALSINH","SATANI FENIL BHARATBHAI","PARMAR DARSHAN PRAFULBHAI","PARSANA FENISHA MANSUKHBHAI","PATEL MANAV MAHESHBHAI","VASANI GAURAV JAYESHBHAI","RADADIYA VIJAY VIPULBHAI","RAIYANI MAYANK VIJAYBHAI","RAIYANI OM MAHESHBHAI","RAIYANI PRINCY HITESHBHAI","VORA BHARGAVKUMAR ATULBHAI","SABHAYA MEET LAVJIBHAI","SARAPADADIYA KRISHNA VIJAYBHAI","BHALALA AKSHAYKUMAR ARVINDBHAI","DHADHAL URVIBEN GAUTAMBHAI","AJANI DARSHAN SURESHBHAI","ASODARIYA SAVAN RAMESHBHAI","BHADANI NIDHI MUKESHBHAI","BHALALA UPEX SURESHBHAI","BHATU AKSHAY NAGABHAI","CHAVADA RANJEET MAVJIBHAI","CHAVDA KOMAL DINESHBHAI","DANGAR DHARMIK BAHADURBHAI","DHANKECHA ARYAN CHANDRESHBHAI","DHANKECHA KIRAJ PARESHBHAI","DHOLAKIYA LENCY SHAILESHBHAI","GAUSWAMI MUKESHNATH KASTURNATH","GOHEL ARCHANA DIPAKBHAI","GONDALIYA PARAM VINODBHAI","GORANIYA NILAM ARAJANBHAI","JADAV KEVIN RAJESHBHAI","JILKA DARSHIT PARESHBHAI","KANOJIYA ADITYA VINAYBHAI","KARENA KAUSHIK JAGDISHBHAI","KHACHAR MAYURJEET KISHORBHAI","LAKHNOTRA UDAY BHAGVANBHAI","LIMBASIYA AVNI BABUBHAI","MAKVANA VIJAY DHARAMSHI","MAKWANA JAYDEEP JENTIBHAI","MANGAROLIYA JASMIN BABUBHAI","MANGROLIYA PURVESH JENTIBHAI"]
    k=5
    for i,j in zip(ab,new):
        i=ttk.Checkbutton(second_frame,cursor="plus", text =i, variable=j,onvalue = 1,bootstyle=SUCCESS)
        i.grid(row=k,column=10,sticky=W,pady=2)
    ...
    def check():
        ...
        for i,j,k in zip(new,ab,number):
            if(i.get()==1):
                connection=mysql.connector.connect(host="localhost",user="root",password="",database="python_final_project")
                mycursor=connection.cursor()
                mycursor.execute("INSERT INTO class_d VALUES (%s, %s,%s,%s)", (k, j,"present",t))
                connection.commit()
            else:
                connection=mysql.connector.connect(host="localhost",user="root",password="",database="python_final_project")
                mycursor=connection.cursor()  
                mycursor.execute("INSERT INTO class_d VALUES (%s, %s,%s,%s)", (k, j,"absent",t))          
                connection.commit()
        tmsg.showinfo("attandance","Saved Successfully")
    ab=Label(second_frame,text="Unclick To Absent Student",justify=CENTER,padx=14,font="verdana  15 bold")
    ab.grid(row=1,column=10,pady=10)
    ab=["VORA YASH VIJAYBHAI","DOBARIYA VISHMAY SHIVABHAI","PANCHANI KUNJKUMAR JAYESHBHAI","HAPANI HETVI","DHOLARIYA PRINCE RAKESHBHAI","HALARI DEV BHAVESHBHAI","NANDINI JOGIA","KASUNDRA DIKSHIT VINOD","VIRAJ KHUNT","SINGH DHANRAJKUMAR SURESHSINGH","KAUSHIK KUMAR","RAUSHAN KUMAR","VINIT KUMAR","VIVEK KUMAR","MOLIYA KRUPAL MUKESHBHAI","NASIT DRINAL BABUBHAI","PADHIYAR JAY PARESHBHAI","PADHIYAR RICHA VIJAYBHAI","SAVSANI MADHAV RAJESHBHAI","VIRADIYA MAYANK ASHVINBHAI","SAVALIYA JENISH GHANSHYAMBHAI","KUNDAN KUMAR","PATORIYA SHANI MAHESHBHAI","RAIYANI DEEP HASMUKHBHAI","RAIYANI KEVIN PARSOTAMBHAI","FEROJ KUJUR","SHINGALA SMITKUMAR JAYSUKHBHAI","VADAVIYA MAHMADJAVID YUNUS","GOVIND KUMAR YADAV","BHOLA YADAV","SANCHANIYA UMANGKUMAR HARESHBHAI","DEEPAK KUMAR SINGH","ARYA","SANNI KUMAR","SEIJAL KUMARI","KOHINOOR KHATOON","ALOK KUMAR","KHUSHI KUMARI","PANDEY GOPAL RAJIV","MUSKAN SHARMA","ASHIQUE HUSSAIN","KRISHNA KUMAR","RAMPYARE KUMAR","VIKASH KUMAR PRAJAPATI","NARENDRA KUMAR SINGH","RAHUL KUMAR SINGH","SURAJ KUMAR","ALI ASRAF","ANUPAM KUMAR GUPTA","ANKIT KUMAR","KUMKUM BHAGAT","ABHISHEK CHAKMA","JUBAYED AHMED","SARITA GURUNG","FEROJ KUJUR","AANAND KUMAR RAUNIYAR"]
    k=5
    for i,j in zip(ab,new):
        i=ttk.Checkbutton(second_frame,cursor="plus", text =i, variable=j,onvalue = 1,bootstyle=SUCCESS)
        i.grid(row=k,column=10,sticky=W,pady=2)
...
def show_attandance_A():     
    ...
    mycursor.execute(f"select * from class_a where date='{t}' ")
    r=mycursor.fetchall()
    num=3
...
def show_attandance_B():     
    ...
    mycursor=connection.cursor()
      # mycursor.execute('insert into vipul values("j ","k"); ')
    mycursor.execute(f"select * from class_b where date='{t}' ")
    r=mycursor.fetchall()
    num=3
    ab1=ttk.Label(second_frame,text="Roll Number",font="time 15 bold",style='warning.Inverse.TLabel',background="yellow")
    ab1.grid(row=1,column=0,padx=0,pady=0)
...
def show_attandance_C():     
    ...
    mycursor=connection.cursor()
      # mycursor.execute('insert into vipul values("j ","k"); ')
    mycursor.execute(f"select * from class_c where date='{t}' ")
    r=mycursor.fetchall()
    num=3
    ab1=ttk.Label(second_frame,text="Roll Number",font="time 15 bold",style='warning.Inverse.TLabel',background="yellow")
    ab1.grid(row=1,column=0,padx=0,pady=0)
...
def show_attandance_D():     
    ...
    mycursor=connection.cursor()
      # mycursor.execute('insert into vipul values("j ","k"); ')
    mycursor.execute(f"select * from class_d where date='{t}' ")
    r=mycursor.fetchall()
    num=3
    ab1=ttk.Label(second_frame,text="Roll Number",font="time 15 bold",style='warning.Inverse.TLabel',background="yellow")
    ab1.grid(row=1,column=0,padx=0,pady=0)
...
def show_present_A():
    ...
    connection=mysql.connector.connect(host="localhost",user="root",password="",database="python_final_project")
    mycursor=connection.cursor()
    mycursor.execute(f"select * from class_a where Status='present' and date='{t}' ")
    r=mycursor.fetchall()
    num=3
    ab1=ttk.Label(second_frame,text="Roll Number",font="time 15 bold",style='warning.Inverse.TLabel',background="yellow")
    ab1.grid(row=1,column=0,padx=0,pady=0)
...
def show_present_B():
    ...
    connection=mysql.connector.connect(host="localhost",user="root",password="",database="python_final_project")
    mycursor=connection.cursor()
    mycursor.execute(f"select * from class_b where Status='present' and date='{t}' ")
    r=mycursor.fetchall()
    num=3
    ab1=ttk.Label(second_frame,text="Roll Number",font="time 15 bold",style='warning.Inverse.TLabel',background="yellow")
    ab1.grid(row=1,column=0,padx=0,pady=0)
...
def show_present_C():
    ...
    connection=mysql.connector.connect(host="localhost",user="root",password="",database="python_final_project")
    mycursor=connection.cursor()
    mycursor.execute(f"select * from class_c where Status='present' and date='{t}' ")
    r=mycursor.fetchall()
    num=3
    ab1=ttk.Label(second_frame,text="Roll Number",font="time 15 bold",style='warning.Inverse.TLabel',background="yellow")
    ab1.grid(row=1,column=0,padx=0,pady=0)
...
def show_present_D():
    ...
    connection=mysql.connector.connect(host="localhost",user="root",password="",database="python_final_project")
    mycursor=connection.cursor()
    mycursor.execute(f"select * from class_d where Status='present' and date='{t}' ")
    r=mycursor.fetchall()
    num=3
    ab1=ttk.Label(second_frame,text="Roll Number",font="time 15 bold",style='warning.Inverse.TLabel',background="yellow")
    ab1.grid(row=1,column=0,padx=0,pady=0)
...
def show_absent_A():
    ...
    connection=mysql.connector.connect(host="localhost",user="root",password="",database="python_final_project")
    mycursor=connection.cursor()
    mycursor.execute(f"select * from class_a where Status='absent' and date='{t}' ")
    r=mycursor.fetchall()
    num=3
    ab1=ttk.Label(second_frame,text="Roll Number",font="time 15 bold",style='warning.Inverse.TLabel',background="yellow")
    ab1.grid(row=1,column=0,padx=0,pady=0)
...
def show_absent_B():
    ...
    connection=mysql.connector.connect(host="localhost",user="root",password="",database="python_final_project")
    mycursor=connection.cursor()
    mycursor.execute(f"select * from class_b where Status='absent' and date='{t}' ")
    r=mycursor.fetchall()
    num=3
    ab1=ttk.Label(second_frame,text="Roll Number",font="time 15 bold",style='warning.Inverse.TLabel',background="yellow")
    ab1.grid(row=1,column=0,padx=0,pady=0)
...
def show_absent_C():
    ...
    connection=mysql.connector.connect(host="localhost",user="root",password="",database="python_final_project")
    mycursor=connection.cursor()
    mycursor.execute(f"select * from class_c where Status='absent' and date='{t}' ")
    r=mycursor.fetchall()
    num=3
    ab1=ttk.Label(second_frame,text="Roll Number",font="time 15 bold",style='warning.Inverse.TLabel',background="yellow")
    ab1.grid(row=1,column=0,padx=0,pady=0)
...
def show_absent_D():
    ...
    connection=mysql.connector.connect(host="localhost",user="root",password="",database="python_final_project")
    mycursor=connection.cursor()
    mycursor.execute(f"select * from class_d where Status='absent' and date='{t}' ")
    r=mycursor.fetchall()
    num=3
    ab1=ttk.Label(second_frame,text="Roll Number",font="time 15 bold",style='warning.Inverse.TLabel',background="yellow")
    ab1.grid(row=1,column=0,padx=0,pady=0)
...
def csv_A():    
    ...
    cursor=db.cursor()
    cursor = db.cursor()
    query = f"SELECT Roll_Number,Student_Name,Status,date from class_a where date='{t}' "
    ...
    df_csv=df.to_csv(f"attendance_of_class_A_{t}.csv")
...
def csv_B(): 
    ...
    cursor=db.cursor()
    cursor = db.cursor()
    query = f"SELECT Roll_Number,Student_Name,Status,date from class_b where date='{t}' "
    cursor.execute(query)
    row = cursor.fetchall()
    w=[]
    x=[]
    ...
    dictionary={'Roll_Number':w, 'Student_Name':x, 'Status':y, 'date': z}
    df=pd.DataFrame(dictionary)
    df_csv=df.to_csv(f"attendance_of_class_B_{t}.csv")
    tmsg.showinfo("Download","Download Successfully")
def csv_C(): 
    global t   
    import pandas as pd
    ...
    cursor=db.cursor()
    cursor = db.cursor()
    query = f"SELECT Roll_Number,Student_Name,Status,date from class_c where date='{t}' "
    cursor.execute(query)
    row = cursor.fetchall()
    w=[]
    x=[]
    ...
    dictionary={'Roll_Number':w, 'Student_Name':x, 'Status':y, 'date': z}
    df=pd.DataFrame(dictionary)
    df_csv=df.to_csv(f"attendance_of_class_C_{t}.csv")
    tmsg.showinfo("Download","Download Successfully")
def csv_D():
    global t    
    import pandas as pd
    ...
    cursor=db.cursor()
    cursor = db.cursor()
    query = f"SELECT Roll_Number,Student_Name,Status,date from class_d where date='{t}' "
    cursor.execute(query)
    row = cursor.fetchall()
    w=[]
    x=[]
    ...
    dictionary={'Roll_Number':w, 'Student_Name':x, 'Status':y, 'date': z}
    df=pd.DataFrame(dictionary)
    df_csv=df.to_csv(f"attendance_of_class_D_{t}.csv")
    tmsg.showinfo("Download","Download Successfully")
